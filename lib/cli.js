...
function traverse (dir) {
  ...
  fs.readdirSync(dir).forEach((file) => {
      ...
      // if yes, traverse it also
      if (fs.statSync(fPath).isDirectory()) {
          if (! options.quiet) {
            console.log('Traversing into '+ fPath);
          }
      ...
      if (fPath.endsWith('.json')) {
        // not a directory so parse the file
        if (! options.quiet) {
          console.log('Parsing ' + file + ' at ' + fPath);
        }
        var json = path.normalize(fPath);
...
function main (args) {
  ...
  if (! options.file && ! options.directory) {
    console.log('Invalid input. Try jsonlint-dir -h for help.');
    process.exit(1);
  }
  if (options.file) {
